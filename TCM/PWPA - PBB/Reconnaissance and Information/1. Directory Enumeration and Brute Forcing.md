...
```shell
# Primary
feroxbuster -u https://target.com -w ~/wordlists/SecLists/Discovery/Web-Content/directory-list-2.3-big.txt -x php,html,js,json,txt,xml,conf,config,bak,backup -C 404,403,429 -t 100 \
  -d 4 \
  --auto-tune \
  --silent \
  -o ferox_results.txt
...
feroxbuster -u https://TARGET \
  -w /usr/share/wordlists/assetnote/raft-medium-words.txt \
  -x php,js,json,html,asp,aspx \
  -t 100 \
  -d 3 \
  --auto-tune \
  -C 404,429 \
...
ffuf -u https://TARGET/FUZZ \
  ...
  -e .bak,.old,.backup,.swp,.swo,.tar,.gz,.zip \
  -t 80 \
  -mc 200,403 \
  -o backups.txt
```
...
1. **First:** `/assetnote/raft-large-words.txt` [AssetNote](https://wordlists.assetnote.io/)
2. **Second:** `/SecLists/Discovery/Web-Content/directory-list-2.3-big.txt`
3. **Third:** Custom wordlist from previous programs
...
##### **1.1.3. 🛡️ Stealth Mode Techniques -- Mimic Legitimate Traffic
```shell

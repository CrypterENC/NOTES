...
## **🏆 Ultimate Directory Enumeration Setup**
#ultimate-directory-enumeration
#### **1. Primary Workhorse : FFUF  + Katana [Crawling discovered endpoints to find linked content]**
...
katana -silent \
  ...
  -o katana_tech.txt
```
#### **4. 📊 POST-PROCESSING & UNIFICATION PIPELINE**
...
echo "  → Admin/Auth: $(wc -l admin_auth.txt)"
...
echo "  → Backup/Configs: $(wc -l backup_configs.txt)"
...
REPORT_FILE="enumeration_report_$(date +%Y%m%d_%H%M%S).md"

cat > "$REPORT_FILE" << EOF
# Directory Enumeration Report
**Target:** $TARGET  
**Date:** $(date)  
**Total URLs Discovered:** $(wc -l all_urls_normalized.txt | awk '{print $1}')  
**Live URLs:** $(wc -l validated_urls.txt | awk '{print $1}')

## 📊 Summary
\`\`\`
$(cat validated_urls.txt | awk '{print $2}' | sort | uniq -c | sort -rn | awk '{print "  "$2": "$1" URLs"}')
\`\`\`

## 🔥 Critical Findings
### Sensitive Files
\`\`\`
$(head -20 sensitive_files.txt)
\`\`\`

### Admin/Auth Endpoints
\`\`\`
$(head -20 admin_auth.txt)
\`\`\`

### API Endpoints
\`\`\`
$(head -20 api_endpoints.txt)
\`\`\`

## 📁 Directory Structure
### Top-level Directories
\`\`\`
$(grep -E "^https?://[^/]+/[^/]+/$" directories.txt | head -20)
\`\`\`

## 🛠 Technologies Detected
\`\`\`
$(cat validated_urls.txt | grep -o "\[.*\]" | sort | uniq -c | sort -rn)
\`\`\`

## 📈 Statistics
- **API Endpoints:** $(wc -l api_endpoints.txt | awk '{print $1}')
- **Admin Panels:** $(wc -l admin_auth.txt | awk '{print $1}')
- **Config Files:** $(wc -l backup_configs.txt | awk '{print $1}')
- **Sensitive Files:** $(wc -l sensitive_files.txt | awk '{print $1}')
EOF
